...
def plot_shock_animated_1d(ntot, w_dir, UNIT_DENSITY, UNIT_LENGTH, UNIT_VELOCITY, datatype, file_name = 'shock_1d.gif', axis = 1, point1 = 0.5, point2 = 0.5, out_dir = ""):
    ...
    Rho = getScalarArray_1d(D.Shock, 1.0, axis, point1, point2)
    ...
    for i in range(ntot - startOffset + 1):
        D = pp.pload(i, varNames=['Shock'], w_dir=w_dir, datatype=datatype)
        Rho = getScalarArray_1d(D.Shock, 1.0, axis, point1, point2)
    ...
    def update(frame_number):
        ...
        D = pp.pload(frame_number, varNames=['Shock'], w_dir=w_dir, datatype=datatype)
        Rho = getScalarArray_1d(D.Shock, 1.0, axis, point1, point2)

        ax.set_xlabel(r'$x~cm$', fontsize=40, fontweight='bold')
        ax.set_ylabel(r'$Shock', fontsize=40, fontweight='bold')
        #ax.set_yscale("log")
        ax.minorticks_on()
