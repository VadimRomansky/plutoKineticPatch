 ...
 ************************************************************************ */
{
    ...
    int sgdims[AL_MAX_DIM], sldims[AL_MAX_DIM], sstarts[AL_MAX_DIM];
  ...
  */
    {
        ...
        MPI_Datatype igsamplingsubarr, ilsamplingsubarr;
...
#ifdef NSAMPLING
        /*-----------------
     *Create global subarray for sparse output
     * */
        for(nd=0;nd<ndim;nd++){
            sgdims[nd]  = s->samplingarrdim[nd];
            sldims[nd]  = s->samplinglarrdim[nd];
            sstarts[nd] = s->samplinggstart[nd];
        }

        MPI_Type_create_subarray(ndim, sgdims, sldims, sstarts,
                                 MPI_ORDER_FORTRAN, s->type, &igsamplingsubarr);
        MPI_Type_commit(&igsamplingsubarr);
        s->gsamplingsubarr = igsamplingsubarr;

        for(nd=0;nd<ndim;nd++){
            sgdims[nd]  = s->samplinglarrdim[nd];
            sldims[nd]  = s->samplinglarrdim[nd];
            sstarts[nd] = s->samplinglstart[nd];
        }

        size_t dsize;

      if(s->type == MPI_DOUBLE){
          dsize = sizeof(double);
      } else if(s->type == MPI_FLOAT){
          dsize = sizeof(float);
      } else if(s->type == MPI_CHAR){
          dsize = sizeof(char);
      } else if(s->type == MPI_UINT16_T){
          dsize = sizeof(unsigned short);
      } else {
          dsize = sizeof(double);
      }

      char ***a = Array3D(s->samplinglarrdim[2], s->samplinglarrdim[1], s->samplinglarrdim[0], dsize);
      if(s->type == MPI_DOUBLE){
          s->sampledArray = (double ***) a;
      } else if(s->type == MPI_FLOAT){
          s->sampledArray = (float ***) a;
      } else if(s->type == MPI_CHAR){
          s->sampledArray = (char***) a;
      } else if(s->type == MPI_UINT16_T){
          s->sampledArray = (unsigned short***) a;
      } else {
          s->sampledArray = (double***) a;
      }
        //printf("sampledArray\n");
        //printf("sampledArray[0][0][0] = %g\n", s->sampledArray[0][0][0]);
        /*---------------------
       * Create local subarray for sparse output
       * -------------------*/
        MPI_Type_create_subarray(ndim, sgdims, sldims, sstarts,
                                 MPI_ORDER_FORTRAN, s->type, &ilsamplingsubarr);
        MPI_Type_commit(&ilsamplingsubarr);
...
#ifdef NSAMPLING
        int startmod = start % NSAMPLING;
        if(startmod == 0){
            startmod = NSAMPLING;
        }
        int endmod = end % NSAMPLING;
        s->samplinglarrdim[i] = (((end - endmod) - (start - startmod + NSAMPLING))/NSAMPLING) + 1;
        s->samplingarrdim[i] = gdim/NSAMPLING;
        if(gdim%NSAMPLING > 0){
            s->samplingarrdim[i] += 1;
        }
        s->samplinggstart[i] = (start/NSAMPLING);
        if(start%NSAMPLING > 0){
            s->samplinggstart[i] += 1;
        }
        s->samplinglstart[i] = 0;
        //printf("rank = %d, dim = %d, arrdim = %d, larrdim = %d, lstart = %d, gstart = %d\n", myrank, i, s->samplingarrdim[i], s->samplinglarrdim[i], s->samplinglstart[i], s->samplinggstart[i]);
