...
#ifdef NSAMPLING
/* ********************************************************************* */
int AL_Set_output_dim(int *sdims, int *sstart, int isz)
/*!
 * Set the dimensions for output for distributed array
 *
 * \param [in]   sdims Array of integers with local dimensions
 * \param [out]  isz   Integer pointer to the input array descriptor
 *
 * \return  AL_SUCCESS if the local dimension are set correctly,
 *          AL_FAILURE otherwise.
 *********************************************************************** */
{
  ...
  */
  ...
  ndim = s->ndim;
  for(i=0;i<ndim;i++){
    s->samplingarrdim[i] = sdims[i];
    s->samplinggstart[i] = sstart[i];
  }
