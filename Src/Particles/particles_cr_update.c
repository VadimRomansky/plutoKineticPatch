 ...
 *********************************************************************** */
{
  ...
  clock_t clock_beg = clock(), clock0;
    ...
    #error CR Particles work in CARTESIAN geometry only (and experimental cylindrical)
#endif
#if (GEOMETRY != CARTESIAN)
    #if PARTICLES_CR_FEEDBACK == YES
        #error CR FEEDBACK work in CARTESIAN geometry only
    #endif
#endif

  //Boundary(data, ALL_DIR, grid); todo why in old pluto?
...
/* --------------------------------------------------------
  ...
  #if PARTICLES_CR_CHECK_RL == YES
  double RL_min = 1.e40, RL_max = 0.0;
  #endif
  ...
  /* --------------------------------------------
    ...
    /* -- A. Get particle 4-velocity and compute \gamma^n -- */
      ...
      double r = p->coord[0];
      double theta = p->coord[1];
      double phi = p->coord[2];
      ...
      double vx = v[0]*sintheta*cosphi - v[1]*costheta*cosphi - v[2]*sinphi;
      double vy = v[0]*sintheta*sinphi - v[1]*costheta*sinphi + v[2]*cosphi;
      double vz = v[0]*costheta - v[1]*sintheta;

      x += dt_half * vx;
      y += dt_half * vy;
      z += dt_half * vz;

      p->coord[0] = sqrt(x*x + y*y + z*z);
      double arccos;
      if(p->coord[0] <= 0){
          arccos = 1.0;
      } else {
      ...
      double vr = vx*sintheta*cosphi + vy*sintheta*sinphi + vz*costheta;
      double vtheta = -vx*costheta*cosphi - vy*costheta*sinphi + vz*sintheta;
      double vphi = -vx*sinphi + vy*cosphi;

      p->speed[0] = vr*gamma;
      p->speed[1] = vtheta*gamma;
      p->speed[2] = vphi*gamma;
#else
#error CR Particles work in CARTESIAN, CYLINDRICAL and SPHERICAL geometry only (and experimental cylindrical)
#endif
...
#elif GEOMETRY == CYLINDRICAL
        B[IDIR] = Particles_Interpolate(data->Vc[iVR], w, p->cell);
        B[JDIR] = Particles_Interpolate(data->Vc[iVZ], w, p->cell);
        B[KDIR] = Particles_Interpolate(data->Vc[iVPHI], w, p->cell);
#elif GEOMETRY == SPHERICAL
       B[IDIR] = Particles_Interpolate(data->Vc[iVR], w, p->cell);
       B[JDIR] = Particles_Interpolate(data->Vc[iVTH], w, p->cell);
       B[KDIR] = Particles_Interpolate(data->Vc[iVPHI], w, p->cell);
#else
#error CR Particles work in CARTESIAN geometry only (and experimental cylindrical)
#endif
...
#elif GEOMETRY == SPHERICAL
    vg[IDIR] = Particles_Interpolate(data->Vc[iVR], w, p->cell);
    vg[JDIR] = Particles_Interpolate(data->Vc[iVTH], w, p->cell);
    vg[KDIR] = Particles_Interpolate(data->Vc[iVPHI], w, p->cell);
#endif
    ...
    /* -- C3. Add resistive field after cleaning step -- */
      ...
      #if    (PHYSICS == MHD) && (RESISTIVITY != NO) || (PHYSICS == ResRMHD)
...
#elif GEOMETRY == CYLINDRICAL
        B[IDIR] = vfluid[iVR]; B[JDIR] = vfluid[iVZ]; B[KDIR] = vfluid[iVPHI];
        E[IDIR] = vfluid[iVR]; E[JDIR] = vfluid[iVZ]; E[KDIR] = vfluid[iVPHI];
#elif GEOMETRY == SPHERICAL
    B[IDIR] = vfluid[iVR]; B[JDIR] = vfluid[iVTH]; B[KDIR] = vfluid[iVPHI];
    E[IDIR] = vfluid[iVR]; E[JDIR] = vfluid[iVTH]; E[KDIR] = vfluid[iVPHI];
#endif
      Bmag2 = DOT_PRODUCT(B,B);
    ...
    /* -- C4. Boris pusher (kick + rotate + kick) -- */
      ...
      #if PARTICLES_CR_CHECK_RL == YES
      for (dir = 0; dir < 3; dir++) u[dir] = p->speed[dir];
      double umag  = sqrt(DOT_PRODUCT(u,u));
      double vmag  = umag/sqrt(1.0 + umag*umag/c2);
      double Bmag  = sqrt(DOT_PRODUCT(B,B));
      double ub    = DOT_PRODUCT(u,B)/(Bmag+1.e-12);
      double uperp = sqrt(umag*umag - ub*ub);
      double RL    = uperp/(PARTICLES_CR_E_MC*Bmag)/(g_domEnd[IDIR]-g_domBeg[IDIR]);
      RL_min = MIN(RL, RL_min);
      RL_max = MAX(RL, RL_max);
      #endif
    ...
    /* -- H. Update spatial coordinate to obtain x^{n+1} -- */
      ...
      v[KDIR] = u[KDIR]*scrh;
            ...
            p->coord[0] = sqrt(pcoord_old[0]*pcoord_old[0] + l*l + 2*pcoord_old[0]*l*cosalpha);
            p->coord[1] = pcoord_old[1] + v[1]*dt_half;
            arccos = (pcoord_old[0]*pcoord_old[0] + p->coord[0]*p->coord[0] -l*l)/(2*pcoord_old[0]*p->coord[0]);
            if((arccos > 1.0) && (arccos < 1.0000000001)){
                arccos = 1.0;
            }
            ...
            deltaPhi = acos(arccos);

            urphi = sqrt(p->speed[0]*p->speed[0] + p->speed[2]*p->speed[2]);
            newalpha = acos(cosalpha) - deltaPhi;
            p->speed[0] = urphi*cos(newalpha);
            if(p->speed[0] != p->speed[0]){
                printLog("p->speed[0] = nan \n");
                QUIT_PLUTO(1);
            ...
            vx = v[0]*sintheta*cosphi - v[1]*costheta*cosphi - v[2]*sinphi;
            vy = v[0]*sintheta*sinphi - v[1]*costheta*sinphi + v[2]*cosphi;
            vz = v[0]*costheta - v[1]*sintheta;
            ...
            vr = vx*sintheta*cosphi + vy*sintheta*sinphi + vz*costheta;
            vtheta = -vx*costheta*cosphi - vy*costheta*sinphi + vz*sintheta;
...
/*  Compute work
    ...
    ( du/dt = E + v/B ).u = u.du/dt = E.u


u(n+1) = u(n) + dt*R


u(n+1/2).(u(n+1) - u(n)) = dt*R(n+1/2).u(n+1/2)

u(n+1/2) = (u(n+1) + u(n))/2


Wc = dt*Ec(n+1/2).u(n+1/2)/(u(n+1/2).(u(n+1) - u(n)))
Wr = dt*Er(n+1/2).u(n+1/2)/(u(n+1/2).(u(n+1) - u(n)))



  W = \int F.ds = \int (Ec + Er).ds = Ec.ds + Er.ds
 
  E.ds =   E[IDIR]*(p->coord[IDIR] - pcoord_old[IDIR])
         + E[JDIR]*....
...
/* ----------------------------------------------------------
   ...
   #if PARTICLES_CR_CHECK_RL == YES
   printf ("RL_min/Lx, RL_max/Lx = %8.3e, %8.3e\n", RL_min, RL_max);
   #endif
