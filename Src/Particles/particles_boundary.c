...
/*!
 * Sets physical boundary conditions on one particle.
 *
 * \param [in]  d         Pointer to PLUTO data structure.
 * \param [in]  grid      Pointer to PLUTO grid structure.
 * \param [in]  p         Pointer to particle
 *********************************************************************** */
{
    int dir, ncopyL, ncopyR;
    double xbeg, xend, L[3];

    DEBUG_FUNC_BEG ("Check Particle Left");

#if PARTICLES == PARTICLES_KIN
#else

    int result = Particles_BoundaryCheck(p, grid);
    return result > 0;
#endif
    DEBUG_FUNC_END ("OneParticles_Boundary");
}
#endif

int FindNumberToExchange(Data * d, Grid *grid)
/*!
 * In parallel mode, find how many particles should be exchanged in the whole box
 *
 * \param [in]   d        Pointer to PLUTO data structure.
...
//todo ifdef or == YES?
    ...
    MPI_Allreduce(&number, &totalNumber, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    return totalNumber;
#endif
#else
    return 0;
 ...
 *********************************************************************** */
{
  ...
  /* -------------------------------------------------------------
    ...
    /* -------------------------------------------------------------
      ...
      for (i = 0; i < nsendL; i++) {
          send_bufL[i] = pstackL[i]->p;
      }
      for (i = 0; i < nsendR; i++) {
          send_bufR[i] = pstackR[i]->p;
      }
    ...
    /* ---------------------------------------------------------------
      ...
      for(i = 0; i < nrecvL; i++){
        ...
        int dir1;
        DIM_LOOP(dir1) {
          double ngh = grid->nghost[dir1];
          double xL  = grid->xbeg[dir1] - grid->dx[dir1][0]*ngh;
          int j   = (int)((p->coord[dir1] - xL)*grid->inv_dx[dir1][IBEG]);

          if (j < 0 || j >= grid->np_tot[dir1]){
            printLog ("! Particles_LocateCell(): particle outside the ");
            printLog ("j = %d dir = %d xp[dir1] = %lf xL = %lf", j, dir1, p->coord[dir1], xL);
