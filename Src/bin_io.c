 ...
 *********************************************************************** */
{
...
#else
    int newNX1 = (NX1 + ioff)/NSAMPLING;
    if((NX1 + ioff)%NSAMPLING > 0){
        newNX1 = newNX1 + 1;
    }
    void* newV = malloc(newNX1*dsize);
    int newNX2 = (NX2 + joff)/NSAMPLING;
    if((NX2 + joff)%NSAMPLING > 0){
        newNX2= newNX2 + 1;
    }
    if(newNX2 == 0){
        newNX2 = 1;
    }
    int newNX3 = (NX3 + koff)/NSAMPLING;
    if((NX3 + koff)% NSAMPLING > 0){
        newNX3 = newNX3 + 1;
    }
    if(newNX3 == 0){
        newNX3 = 1;
    ...
    int kcounter = 0;
    k = KBEG;
    //for (k = KBEG; k <= KEND + koff; k = k + NSAMPLING) {
    while(kcounter < newNX3){
        int jcounter = 0;
        j = JBEG;
        //for (j = JBEG; j <= JEND + joff; j = j + NSAMPLING) {
        while(jcounter < newNX2){
            i = IBEG + (NX1_TOT + ioff)*(j + (NX2_TOT + joff)*k);
            int counter = 0;
            int ii = IBEG;
            while(counter < newNX1){
                //for (int ii = IBEG; ii <= IEND + ioff; ii = ii + NSAMPLING){
                ((double*)newV)[counter] = ((double*)Vc)[i + ii - IBEG];
                counter++;
                ii = ii + NSAMPLING;
                //}
            }
            fwrite (newV, dsize, newNX1, fl);
            jcounter++;
            j = j + NSAMPLING;
        }
        kcounter++;
