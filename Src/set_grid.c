...
*/
   ...
   ---------------------------------------------- */
    ...
    if (prank == 0){
...
#else
        DIM_LOOP(idim){
            int n2 = grid->np_int_glob[idim]/NSAMPLING;
            if(grid->np_int_glob[idim] % NSAMPLING > 0){
                n2 = n2 + 1;
            }
            fprintf (fg, "# X%d: [% f, % f], %d point(s), %d ghosts\n", idim+1,
                     g_domBeg[idim], g_domEnd[idim],
        ...
        for (idim = 0; idim < 3; idim++) {
            ...
            ibeg = grid->gbeg[idim];
            iend = grid->gend[idim];
            int n1 = (iend - ibeg + 1)/NSAMPLING;
            if((iend - ibeg + 1) % NSAMPLING > 0){
                n1 = n1 + 1;
            }
            if(n1 == 0){
                n1 = 1;
            ...
            i = ibeg;
            while(counter < n1){
                //for (i = ibeg; i <= iend; i = i + NSAMPLING) {
                fprintf (fg, " %d   %18.12e    %18.12e\n",
                         i-ngh+1, grid->xl_glob[idim][i], grid->xr_glob[idim][i]);
                counter++;
                i = i + NSAMPLING;
   ...
   ---------------------------------------------- */
    ...
    int  end[3] = {0,0,0};
#ifdef PARALLEL
    AL_Get_bounds  (SZ1, beg, end, grid->nghost, AL_C_INDEXES);
#else
    for (idim = 0; idim < 3; idim++) {
