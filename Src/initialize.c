...
double p_grid_min = P_GRID_MIN;
double p_grid_max = P_GRID_MAX;
#endif
...
/* SetDistributedArray (SZ1, type, gsize, periods, stagdim);
  return args: beg, end, lsize, lbeg, lend, gbeg, gend, is_gbeg, is_gend
*/

  AL_Sz_init (MPI_COMM_WORLD, &SZ1);
  AL_Set_type (MPI_DOUBLE, 1, SZ1);
  AL_Set_dimensions (DIMENSIONS, SZ1);
  AL_Set_global_dim (gsize, SZ1);
  AL_Set_ghosts (ghosts, SZ1);
  AL_Set_periodic_dim (periods, SZ1);
  AL_Set_parallel_dim (pardim, SZ1);

  AL_Decompose (SZ1, procs, decomp_mode);
  AL_Get_local_dim (SZ1, lsize);
  AL_Get_bounds (SZ1, beg, end, ghosts, AL_C_INDEXES);
  AL_Get_lbounds (SZ1, lbeg, lend, ghosts, AL_C_INDEXES);
  AL_Get_gbounds (SZ1, gbeg, gend, ghosts, AL_C_INDEXES);
  AL_Is_boundary (SZ1, is_gbeg, is_gend);
...
#if PARTICLES == PARTICLES_KIN
    AL_Sz_init (MPI_COMM_WORLD, &SZ_F);
    AL_Set_type (MPI_DOUBLE, 1, SZ_F);
    AL_Set_dimensions (DIMENSIONS, SZ_F);
    AL_Set_global_dim (gsize, SZ_F);
    AL_Set_ghosts (ghosts, SZ_F);
    AL_Set_periodic_dim (periods, SZ_F);
    AL_Set_parallel_dim (pardim, SZ_F);

    AL_Decompose (SZ_F, procs, decomp_mode);
    AL_Get_local_dim (SZ_F, lsize);
    AL_Get_bounds (SZ_F, beg, end, ghosts, AL_C_INDEXES);
    AL_Get_lbounds (SZ_F, lbeg, lend, ghosts, AL_C_INDEXES);
    AL_Get_gbounds (SZ_F, gbeg, gend, ghosts, AL_C_INDEXES);
    AL_Is_boundary (SZ_F, is_gbeg, is_gend);
#endif
...
/* ----------------------------------------------
  ...
  data->gmresBasis = (LargeVectorBasis*) malloc(1*sizeof(LargeVectorBasis));
  //(*(data->gmresBasis)) = createLargeVectorBasis(5, NX3_TOT, NX2_TOT, NX1_TOT, NMOMENTUM);
  createLargeVectorBasis1(data->gmresBasis, 5, NX3_TOT, NX2_TOT, NX1_TOT, NMOMENTUM);
#endif

#if (PARTICLES == PARTICLES_KIN) || (TURBULENT_FIELD == YES)
  data->Jkin1 = ARRAY_4D(NX3_TOT, NX2_TOT, NX1_TOT, NMOMENTUM, double);
  data->Jkin2 = ARRAY_4D(NX3_TOT, NX2_TOT, NX1_TOT, NMOMENTUM, double);
  data->Jkin3 = ARRAY_4D(NX3_TOT, NX2_TOT, NX1_TOT, NMOMENTUM, double);

  data->p_grid = ARRAY_1D(NMOMENTUM, double);
  //double factor1 = pow(p_grid_max/p_grid_min, 1.0/(NMOMENTUM - 1));
  //data->p_grid[0] = p_grid_min;
  //for(int i = 1; i < NMOMENTUM; ++i){
  //    data->p_grid[i] = data->p_grid[i-1]*factor1;
  //}
#endif
  ...
  for(int i = 1; i < NTURB; ++i){
      data->k_turb[i] = data->k_turb[i-1]*factor;
  }
  data->turbulent_matrix = (MatrixElementNode*****) malloc(NX3_TOT*sizeof(MatrixElementNode****));
  for(int k = 0; k < NX3_TOT; ++k){
      data->turbulent_matrix[k] = (MatrixElement****) malloc(NX2_TOT*sizeof(MatrixElementNode***));
      for(int j = 0; j < NX2_TOT; ++j){
          data->turbulent_matrix[k][j] = (MatrixElement***) malloc(NX1_TOT*sizeof(MatrixElementNode**));
          for(int i = 0; i < NX1_TOT; ++i){
              data->turbulent_matrix[k][j][i] = (MatrixElement**) malloc(NTURB*sizeof(MatrixElementNode*));
              for(int l = 0; l < NTURB; ++l){
                  data->turbulent_matrix[k][j][i][l] = NULL;
              }
          }
      }
  }
