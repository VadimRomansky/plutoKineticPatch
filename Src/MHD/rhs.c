...
#else
 #if (defined FINITE_DIFFERENCE) || (defined HIGH_ORDER)
...
void RightHandSide (const Sweep *sweep, timeStep *Dts, 
                    int beg, int end, double dt, Grid *grid, Data* data)
/*! 
 *
...
/* --------------------------------------------------------
  ...
  for (i = beg; i <= end; i++) {
    ...
    #if RADIATION
    NMHD_LOOP(nv) rhs[i][nv] = -scrh*(flux[i][nv] - flux[i-1][nv]);
    #if IRRADIATION
    rhs[i][FIR] = 0.;
    #endif
    #else
  ...
  if (g_dir == IDIR){
    ...
    /* ------------------------------------------
      ...
      #if RADIATION
      NMHD_LOOP(nv) rhs[i][nv] = -dtdV*(fA[i][nv] - fA[i-1][nv]); 
      #if IRRADIATION
      rhs[i][FIR] = 0.;
      #endif
      #else
    ...
    /* ------------------------------------------
      ...
      #if RADIATION
      NMHD_LOOP(nv) rhs[j][nv] = -dtdV*(fA[j][nv] - fA[j-1][nv]);
      #if IRRADIATION
      rhs[j][FIR] = 0.;
      #endif
      #else
    ...
    /* ------------------------------------------
      ...
      #if RADIATION
      NMHD_LOOP(nv) rhs[k][nv] = -dtdV*(fA[k][nv] - fA[k-1][nv]);
      #if IRRADIATION
      rhs[k][FIR] = 0.;
      #endif
      #else
...
#ifndef HIGH_ORDER
  RightHandSideSource (sweep, Dts, beg, end, dt, phi_p, grid, data);
#endif

#if SCALAR_ADVECTION == YES
  for (i = beg; i <= end; i++){
    for (nv = 1; nv < NVAR; nv++) sweep->rhs[i][nv] = 0.0;
  }
#endif
